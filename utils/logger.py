        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                try:
                    ...
                    actual_logger.info(f"EXITING {func_name} (result logging failed: {e})")
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                if args and hasattr(args[0], 'dict'):
                    ...
                    actual_logger.info(f"REQUEST to {func_name}: {safe_json_serialize(request_data)}")
                else:
                    actual_logger.info(f"REQUEST to {func_name} with args: {repr(args)}, kwargs: {repr(kwargs)}")
            except Exception as e:
            ...
            try:
                ...
                try:
                    if hasattr(result, 'dict'):
                        ...
                        actual_logger.info(f"RESPONSE from {func_name}: {safe_json_serialize(response_data)}")
                    else:
                        result_str = safe_json_serialize(result)
                        actual_logger.info(f"RESPONSE from {func_name}: {result_str}")
                except Exception as e:
                    actual_logger.warning(f"Failed to log response for {func_name}: {e}")
