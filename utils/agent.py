...
def get_answers(challenge_request: ChallengeRequest) -> ChallengeResponse:
    logger.info(f"Processing challenge request with {len(challenge_request.questions)} questions")
    logger.info(f"Target URL: {challenge_request.url}")
    
    answers = []
    for i, question in enumerate(challenge_request.questions, 1):
        logger.info(f"Processing question {i}/{len(challenge_request.questions)}: {question}")
